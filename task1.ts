...
function drawRadius(ctx: CanvasRenderingContext2D, station: Station): void {
  ctx.beginPath();
  ctx.arc(station.coordinates[0], station.coordinates[1], station.radius, 0,Math.PI*2,true); // Внешняя окружность
  ctx.stroke();
}

function drawStationCenter(ctx: CanvasRenderingContext2D, station: Station): void {
  ctx.beginPath();
...
function distance ( station: Station, point: number[]): number {
  return ((station.coordinates[0] - point[0]) ** 2 + (station.coordinates[1] - point[1]) ** 2 )
}

function onRing( station: Station, point: number[] ): boolean {
...
function tracking(stations: Station[], stationIds: number[][]) {
  ...
  stations.forEach(station => { // Отрисовка странций и радиусов
  ...
  stationIds.forEach( (visibleArr, index) => {
      ...
      let currentPoints: number[][] = JSON.parse(JSON.stringify(intersectionPoints)) // точки между которыми будет типсон 
      /* 
      ...
      */
      ...
      if ( visibleArr.length === 2 ) {
          visibleArr.forEach( (circleId: number) => {
              let circle: Station | undefined;
                  circle = stations.find(station => station.id === circleId)
              const [ x, y ] = [ circle?.coordinates[0], circle?.coordinates[1] ]
              const R = circle?.radius
              if( x && y && R ) {
                currentPoints = currentPoints.filter( (point: number[]) => {
      ...
      currentPoints.forEach( (point: number[]) => { // отрисовываю точки для наглядности
          ctx.beginPath();
          ctx.arc(point[0], point[1], 3, 0, Math.PI*2,true); // Внешняя окружность
          ctx.fill();
      })

      const partiсipantStations = stations.filter( station => { // все станции границы которых хоть как-то зайдействованы 
          let flag = false
          currentPoints.forEach((point: number[]) => {
      ...
      partiсipantStations.forEach((station, i) => {
          ...
          let stationX = station.coordinates[0],
              stationY = station.coordinates[1],
              [ x1, y1 ] = [ points[0][0], points[0][1] ],
              [ x2, y2 ] = [ points[1][0], points[1][1] ],
              counterclockwise = false,
              angleFrom: number = Math.atan2(y2 - stationY, x2 - stationX), // угол от абсциссы до первой точки 
              angleTo: number = Math.atan2(y1 - stationY, x1 - stationX), // угол от абсциссы до второй точки
              angle = null,
              r = station.radius
      ...
      ctx.moveTo(...(currentPoints.at(-currentPoints.length) as [ number, number ]))
      ...
      for ( let i = -currentPoints.length + 1; i < 0; i++ ){ 
          ...
          ctx.lineTo(...(curPoint as [ number, number ]))
      }
      ctx.stroke()
      ctx.fill()
  })
